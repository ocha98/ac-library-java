...
public class ConvolutionTest {
    ...
    public void Test() {
        ...
        for (int i = 0; i < 1000; i++) {
            ...
            List<ModIntFactory.ModInt> va =
                    Arrays.stream(a).mapToObj(factory::create).collect(Collectors.toList());
            List<ModIntFactory.ModInt> vb =
                    Arrays.stream(b).mapToObj(factory::create).collect(Collectors.toList());
            long[] expected = Convolution.convolutionNaive(a, b, mod);
            long[] actual = Convolution.convolution(va, vb).stream()
                    .mapToLong(ModIntFactory.ModInt::value)
                    .toArray();
